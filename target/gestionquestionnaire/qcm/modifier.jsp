...
<head>
    <meta charset="UTF-8">
    <title>Gestion Questionnaire - Modifier QCM</title>
...
<body>
    ...
    <div class="main-content">
        ...
        <form action="qcm" method="post">
            ...
            <input type="hidden" name="id" value="${qcm.numQuest}"/>

            <label for="niveau">Niveau:</label>
            <select id="niveau" name="niveau" required>
                <option value="">-- Sélectionnez un niveau --</option>
                <c:forEach var="n" items="${niveaux}">
                    <option value="${n}" <c:if test="${qcm.niveau == n}">selected</c:if>>${n}</option>
                </c:forEach>
            </select><br/>
            ...
            <select id="theme" name="theme">
                ...
                <c:forEach var="t" items="${themes}">
                    <option value="${t}" <c:if test="${qcm.theme == t}">selected</c:if>>${t}</option>
                </c:forEach>
            </select>
            <input type="text" id="themeNew" name="themeNew" placeholder="Ou tapez un nouveau thème"/><br/>

            <label for="question">Question:</label>
            <textarea id="question" name="question" rows="2" required>${qcm.question}</textarea><br/>

            <label for="reponse1">Réponse 1:</label>
            <input type="text" id="reponse1" name="reponse1" value="${qcm.reponse1}" required/><br/>

            <label for="reponse2">Réponse 2:</label>
            <input type="text" id="reponse2" name="reponse2" value="${qcm.reponse2}" required/><br/>

            <label for="reponse3">Réponse 3:</label>
            <input type="text" id="reponse3" name="reponse3" value="${qcm.reponse3}" required/><br/>

            <label for="reponse4">Réponse 4:</label>
            <input type="text" id="reponse4" name="reponse4" value="${qcm.reponse4}" required/><br/>

            <label for="bonneReponse">Bonne Réponse (1-4):</label>
            <input type="number" id="bonneReponse" name="bonneReponse" min="1" max="4" value="${qcm.bonneReponse}" required/><br/>

            <div style="margin-top: 30px;">
                <button type="submit" id="submitBtn"><i class="fas fa-save"></i> Modifier le QCM</button>
